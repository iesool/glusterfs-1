                 ...
                 unsigned long count, char *name)
{
		...
		//每个块大小为sizeof_type(可用大小) + GF_MEM_POOL_PAD_BOUNDARY(存放管理信息)
 ...
 * 从内存池获取一个内存块
 * 
 * 如果内存池内存块用完，则从系统内存分配一块，并把该内存块归属在该mem_pool，但不加入链表
 *
 */
void *
mem_get (struct mem_pool *mem_pool)
{
        ...
        LOCK (&mem_pool->lock);
        {
                 ...
                 *当内存不足时，它向系统申请了内存，并处理了内存的管理信息后，直接将内存返回给调用者
                 */
                 /* 未从内存池申请到内存块，则重新从系统申请内存
            	 * 该内存归属到mem_pool中，但未加入 mem_pool 的 list 链表中
             	 */
 ...
 * 如果是从系统内存分配，则直接释放，并把 mem_pool->curr_stdalloc 计数减一
 */
void
mem_put (void *ptr)
{
