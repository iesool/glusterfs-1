...
typedef struct clnt_conf {
	...
	gf_boolean_t           quick_reconnect; /* When reconnecting after
        ...
        gf_boolean_t           filter_o_direct; /* if set, filter O_DIRECT from
                                                   the flags list of open(),strict-o-direct off, ignore the O_DIRECT flag. */ 
		gf_boolean_t 		   filter_o_directory;/*if set , filter O_DIRECTORY from the flags list of open/
