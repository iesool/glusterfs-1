              ...
              mode_t mode, mode_t umask, dict_t *xdata)
{
        ...
        if (proc->fn)
                ret = proc->fn (frame, this, &args);
		//client_symlink(call_frame_t * frame,xlator_t * this,const char * linkpath,loc_t * loc,mode_t umask,dict_t * xdata);
		//symlink();
out:
        if (ret)
               ...
               mode_t mode, mode_t umask, fd_t *fd, dict_t *xdata)
{
        ...
        if (!conf->filter_o_direct)
                args.flags = flags;
		//network.remote-dio disable 不过滤O_DIRECT,即对原有flags不做处理
        else
                args.flags = (flags & ~O_DIRECT);
		//network.remote-dio enable  检查flags中O_DIRECT标志位置为不直接IO，经过缓存。
